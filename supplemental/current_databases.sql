...
(2.1, 3.99, 3.49),
(3.1, 4.99, 4.49);
INSERT INTO inventory (number) SELECT number FROM csci467.parts;
...
(
   1, 1.0,
   (SELECT shipping FROM extra_charge WHERE weight = 1.1),
   (SELECT handling FROM extra_charge WHERE weight = 1.1),
   10.11, (SELECT CURDATE()), 'in progress'
),
(
   2, 2.0,
   (SELECT shipping FROM extra_charge WHERE weight = 2.1),
   (SELECT handling FROM extra_charge WHERE weight = 2.1),
   20.22, (SELECT CURDATE()), 'in progress'
),
(
   3, 3.0,
   (SELECT shipping FROM extra_charge WHERE weight = 3.1),
   (SELECT handling FROM extra_charge WHERE weight = 3.1),
...
-- give the other 2 employees random position that is not admin
DELIMITER //
FOR i IN 21 .. 22
DO
   INSERT INTO position_collection
